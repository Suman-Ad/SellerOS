                    ...
                    .forEach(
                        ([internalField, csvColumn]) => {
                            ...
                            if (
                                ...
                                normalizedRow.bankSettlementPrice =
                                    row["BANK SETTLEMENT PRICE"] ||
                                    row["Bank Settlement Price"] ||
...
Duplicates Skipped: ${skipped}
    ...
    const executePricingImport = async () => {
        ...
        for (
            ...
            // only skip explicit reject
            if (
                action &&
                action.trim().toUpperCase() === "REJECT"
            ) {
                continue;
            }
            ...
            const meeshoPrice =
                Number(
                    String(row.meeshoPrice || "")
                        .replace(/[^0-9.]/g, "")
                );

            const bankSettlementPrice =
                Number(
                    String(row.bankSettlementPrice || "")
                        .replace(/[^0-9.]/g, "")
                );

            if (!meeshoPrice || !bankSettlementPrice) {
                continue;
            }

            const deliveryCost =
                Number(
                    (meeshoPrice - bankSettlementPrice)
                        .toFixed(2)
                );
